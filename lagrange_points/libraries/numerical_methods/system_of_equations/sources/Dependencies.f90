...
implicit none
    ...
    abstract interface  
       ...
       real function F_1D_IVBP(x, t, u, ux, uxx) 
            real, intent(in) :: x, t, u, ux, uxx                 
       end function 
        real function F_2D_IVBP(x, y, t, u, ux, uy, uxx, uyy, uxy) 
...
function Function_Dependencies( F, N, P_ux ) result(D)
    ...
    integer :: i, j
    real :: h( N ), dF, u( N ), dx, r(2), Daux( 1:N, 1:2)
...
!*****     
function Dependencies_IVBP_1D( F ) result(d)
    procedure (F_1D_IVBP) :: F
    logical :: d( 2 )
...
function Dependencies_IVBP_2D( F ) result(D)
    procedure (F_2D_IVBP) :: F
    logical :: D(5)
